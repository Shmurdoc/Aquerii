...
class RoomManager {
    ...
    roomKey(workspaceId, resourceType, resourceId) {
        return `${resourceType}:${workspaceId}:${resourceId}`;
    }
    async join(socket, payload, resourceType, resourceId) {
    ...
    async leave(socket, resourceType, resourceId, workspaceId) {
        ...
        const stillPresent = sockets.some((s) => s.data.user?.sub === socket.data.user?.sub && s.id !== socket.id);
        if (!stillPresent) {
    ...
    async leaveRoom(socket, room) {
        ...
        const stillPresent = sockets.some((s) => (s.data.user?.sub ?? s.data.userId) === userId && s.id !== socket.id);
