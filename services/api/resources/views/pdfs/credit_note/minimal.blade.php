    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .t-row.final {
            ...
            font-weight: 700; color: #dc2626; padding-top: 12px; margin-top: 4px;
        }
...
<body>
    ...
    <div class="page">
        ...
        <div class="info-grid">
            ...
            <div class="info-block">
                ...
                <div class="i-detail">
                    Date: {{ \Carbon\Carbon::parse($document->issue_date)->format('d M Y') }}<br>
                    @if(!empty($document->invoice_number))Invoice: {{ $document->invoice_number }}<br>@endif
                    Reason: {{ $document->credit_reason ?? 'N/A' }}@if(!empty($document->currency))<br>Currency: {{ $document->currency }}@endif
                </div>
            </div>
        </div>
    ...
    <div class="totals-wrapper" style="padding: 0 48px 0;">
        <div class="totals-inner">
            <div class="t-row">
                <span>Subtotal</span>
                <span class="tv">({{ number_format(abs($document->subtotal), 2) }})</span>
            </div>
            @if(!empty($document->tax_total) && $document->tax_total > 0)
            <div class="t-row">
                <span>Tax</span>
                <span class="tv">({{ number_format(abs($document->tax_total), 2) }})</span>
            </div>
            @endif
            <div class="t-row final">
                <span>Credit Total</span>
                <span class="tv">({{ number_format(abs($document->total), 2) }})</span>
            </div>
        </div>
    </div>
