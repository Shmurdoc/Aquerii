...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .body { padding: 32px 40px 0; position: relative; }
        ...
        .totals-table { width: 280px; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden; }

        .totals-row {
            display: flex; justify-content: space-between; padding: 9px 16px;
            border-bottom: 1px solid #f3f4f6; font-size: 13px; color: #6b7280;
        }

        .totals-row:last-child { border-bottom: none; }

        .totals-row.total-final {
            background: var(--accent); font-weight: 700; font-size: 15px; color: #ffffff;
        }
...
<body>
    ...
    <div class="header-band">
        ...
        <div class="doc-label">
            ...
            @php $statusClass = 'status-' . ($document->status ?? 'cleared'); @endphp
            <span class="status-badge {{ $statusClass }}">{{ ucfirst($document->status ?? 'cleared') }}</span>
        </div>
    </div>
    ...
    <div class="body">
        ...
        <div class="meta-strip">
            <div class="meta-cell">
                ...
                <div class="value">{{ \Carbon\Carbon::parse($document->payment_date ?? $document->issue_date)->format('d M Y') }}</div>
            </div>
            <div class="meta-cell">
                <div class="label">Payment Method</div>
                <div class="value">{{ $document->payment_method ?? 'N/A' }}</div>
            </div>
            <div class="meta-cell">
                <div class="label">Reference</div>
                <div class="value">{{ $document->reference ?? $document->number }}</div>
            </div>
        </div>
    ...
    <div class="totals-wrapper" style="padding: 0 40px 0;">
        <div class="totals-table">
            ...
            <div class="totals-row total-final">
                ...
                <span class="t-value paid-amount">{{ number_format($document->total, 2) }}</span>
            </div>
        </div>
    </div>
