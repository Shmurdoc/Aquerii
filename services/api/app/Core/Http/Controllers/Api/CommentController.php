...
class CommentController extends Controller
{
    ...
    public function store(Request $request, Workspace $workspace, string $itemId): JsonResponse
    {
        ...
        // Notify mentioned users — detect @{uuid} patterns in body
        $this->dispatchMentionNotifications($workspace->id, $itemId, $validated['body'], $request->user()->id);

        return response()->json(['data' => ['id' => $id]], 201);
    }

    private function dispatchMentionNotifications(string $workspaceId, string $itemId, string $body, string $authorId): void
    {
        // Match @followed-by-uuid or @username patterns; we support UUID mentions
        preg_match_all('/@([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i', $body, $matches);

        $mentionedIds = collect($matches[1] ?? [])->unique()->reject(fn ($id) => $id === $authorId);
