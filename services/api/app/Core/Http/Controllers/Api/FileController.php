...
class FileController extends Controller
{
    ...
    {
        $request->validate([
            'file' => 'required|file|max:102400|mimes:jpg,jpeg,png,gif,webp,svg,pdf,doc,docx,xls,xlsx,csv,txt,zip,mp4,mp3,wav',
        ]);
        ...
        $used = (int) DB::table('workspaces')->where('id', $workspace->id)->value('storage_used_bytes');
        $limit = (int) ($workspace->storage_quota_bytes ?? 5368709120);
        abort_if($used + $size > $limit, 402, 'Storage quota exceeded.');

        $path = "workspaces/{$workspace->id}/items/{$itemId}/".Str::uuid().'_'.$safeName;
        Storage::disk('s3')->put($path, $file->getContent());

        $id = Str::uuid()->toString();
        DB::transaction(function () use ($id, $itemId, $workspace, $safeName, $path, $size, $file) {
        ...
        return response()->json(['data' => ['id' => $id, 'filename' => $safeName]], 201);
    }
