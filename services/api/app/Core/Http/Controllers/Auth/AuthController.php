...
class AuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        [$user, $workspace, $token, $expiresAt] = $this->authService->register($data);
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        if ($result === 'MFA_REQUIRED') {
            $mfaToken = $this->authService->generateMfaToken($request->input('email'));
    ...
    public function verifyMfaWithToken(Request $request): JsonResponse
    {
        $data = $request->validate([
        ...
        $result = $this->authService->verifyMfaWithToken($data['mfa_token'], $data['code']);

        if (! $result) {
            return response()->json([
                'error' => ['code' => 'MFA_INVALID', 'message' => 'Invalid or expired MFA token or code.'],
            ], 422);
        }
    ...
    public function refresh(Request $request): JsonResponse
    {
        ...
        [$token, $expiresAt] = $this->authService->refreshToken($request->input('token'));

        return response()->json(['data' => ['token' => $token, 'expires_at' => $expiresAt]]);
    }
    ...
    public function resendVerification(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => ['message' => 'If that email exists and is unverified, a verification link has been sent.']]);
    }
