...
class EnforceIdempotency
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (! $cached) {
            ...
            if ($dbRecord) {
                // PDO + pgsql may auto-decode JSONB columns to a PHP array;
                // json_decode(array, true) returns null — handle both cases.
                $rawResponse = $dbRecord->response;
                $decodedResponse = is_array($rawResponse)
                    ? $rawResponse
